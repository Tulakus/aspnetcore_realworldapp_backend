...
namespace realworldapp.Handlers.Users
{
        ...
        IRequestHandler<UpdateUserCommand, UserWrapper>, IRequestHandler<QueryUserInfoCommand, UserWrapper>
    {
        ...
        public UserHandler(AppDbContext context, IPasswordHashProvider passwordHashProvider, IJwt jwtService, ICurrentUserAccessor currentUserAccessor)
        {
            _context = context;
        ...
        {
            var claims = new[] { new Claim(ClaimTypes.Email, user.Email), new Claim("username", user.Username) };
            return claims;
        }
